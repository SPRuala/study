package com.example.makeexceptionandinterceptorfront.controller.interceptor;

import com.example.makeexceptionandinterceptorfront.exception.GraceException;
...
public class StudentInterceptor implements HandlerInterceptor {
    ...
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) throws Exception {
        ...
        if (StringUtils.isEmpty(sid) || StringUtils.isEmpty(studentToken)){
            ...
            GraceException.displayException("用户校验不通过,信息不完整");
            //拦截
            return false;
        }
        ...
        if (!sid.equalsIgnoreCase("1001") || !studentToken.equalsIgnoreCase("abcxyz")){
            log.error("用户权限不通过");
            GraceException.displayException("用户权限不通过");
            //拦截
            return false;
        }
